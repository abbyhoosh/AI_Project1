...
#algorithm for minimax game play
#currDepth is the current depth in the tree
#nodeNum is the current node number
#isMax is a boolean that tells if it is Max or Min turn
#listScores is a list of scores
#maxDepth is the max depth that the tree goes/will search
def minimax(currDepth, nodeNum, isMax, listScores, maxDepth):
    if(currDepth == maxDepth):
        return listScores[nodeNum]
    if(isMax):
        return max(minimax(currDepth +1, nodeNum*2, False, listScores, maxDepth),
                   minimax(currDepth+1, nodeNum*2+1, False, listScores, maxDepth))
    else:
        return min(minimax(currDepth +1, nodeNum*2, True, listScores, maxDepth),
                   minimax(currDepth+1, nodeNum*2+1, True, listScores, maxDepth))
